	...
	muniApp.config(function($routeProvider) {
		$routeProvider
			...
			.when('/CRUD_task', {
				templateUrl : 'pages/crud_operation.html',
				controller  : 'Book'
			});
	});

// header and footer directives
muniApp.directive('header', function () {
	return {
		restrict: 'A',
		replace: true,
		scope: {user: '='},
		templateUrl: "/Tasks/pages/header.html",
...
muniApp.directive('footer', function () {
	return {
		restrict: 'A',
		replace: true,
		scope: {user: '='},
		templateUrl: "/Tasks/pages/footer.html",
		controller: ['$scope', '$filter', function ($scope, $filter) {
			$scope.message = 'My APP';
		}]
	};
...
$scope.getData();
	...
	muniApp.controller("Book", function ($scope) {
        $scope.items = [];
        ...
        $scope.removeItem = function (index) {
			var _index = $scope.items.indexOf(index);  
